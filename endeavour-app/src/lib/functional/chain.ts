 ...
 */
import { reverse } from '@/lib/functional/reverse';
 ...
 *      Returns a function that transforms a value A into a result R.
 */
export function chain(...fns: ((i: unknown) => unknown)[]): (value: unknown) => unknown;
export function chain(...fns: ((i: unknown) => unknown)[]): (value: unknown) => unknown {
  if (fns.length === 0) {
    return v => v;
  }
  ...
  const rfns = reverse(fns);
  return (value: unknown) => rfns.reduce((prev, fn) => fn(prev), value);
}
